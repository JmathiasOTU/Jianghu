...
- **15 states.** Ground: `Idle`, `Walk`, `Run`, `Sprint`, `CrouchIdle`, `CrouchWalk`, `Slide`, `HardLanding`. Air: `Airborne`, `SlideJump`, `DoubleJump`. Wall: `WallRun`, `WallLeap`, `WallCling`, `WallBoost`. Ladders and swimming use the Humanoid's own states.
...
- **Not built:** Vault, Dash, a Qinggong/stamina resource, combat interaction (§14).
...
| `Shared/Movement/SpatialQueries.luau` | Pure geometry reads: ground, support, wall run, cling wall, climbable, water, floor-below |
...
| Space, airborne | In order: `WallLeap` (if wall-running), else `WallRun`, else `WallCling`, else `DoubleJump`. The first legal one wins, so a nearby wall never consumes the double jump |
...
3. Moving forward with `preAirborneLocomotion` set: back into Run/Sprint.
...
- Fall height survives Airborne↔DoubleJump/WallRun/WallLeap/WallCling/WallBoost hops, so a cling partway down a long fall doesn't erase it. Swimming resets it.
...
| Hold | Zero velocity (full-force override, cancels gravity); Space does nothing while clinging |
...
| vs WallLeap | Can't cling to the wall you just leapt from until you land |
...
| SlideJump | Airborne |
| Airborne | Grounded set, DoubleJump, WallRun, WallCling |
| DoubleJump | Airborne, grounded set |
| HardLanding | Grounded set (minus itself), Airborne |
| WallRun | Airborne, grounded set, WallLeap |
| WallLeap | Airborne |
| WallCling | Airborne, WallBoost, grounded set |
| WallBoost | Airborne, grounded set |
...
- **`CharacterMover`** is the only code that touches the Humanoid/root part:
  ...
  - `SetHorizontalVelocity`/`SetVerticalVelocity` are one-shot writes for launches that hand off in the same tick (SlideJump, WallLeap, WallBoost, cling drop). A constraint there would never get its `Exit` to clear it.
...
| `ClaimTraversalMove` | C→S | `DoubleJump`, `WallRun`, `WallLeap`, `WallCling`, `WallBoost` | 5/s |
...
| Idle, Walk | 18 |
| Run | 28 |
| Sprint, Airborne, DoubleJump, WallLeap, WallBoost | 40 |
...
- **Airborne allowance** (`airborneAllowedSpeed`): WallLeap (the exact bound, about 166) and SlideJump (50) raise the air cap for the whole flight, since nothing slows horizontal flight.
...
- **Raises:** each accepted move adds its maximum rise and moves the apex time: DoubleJump (`DoubleJumpMaxRise`, `DoubleJumpTimeToApex`), WallLeap and WallBoost (ballistic), WallRun entry snap, `VerticalAllowanceGranted`. Launches replace vertical velocity, so "old ceiling + rise" is a true bound.
...
Always on in Studio only: `[MovementViolation]` for every correction, `[TraversalClaim] ... REJECTED` for DoubleJump/WallBoost claims.
...
- **Tunable (F4):** speed tiers and jump; Run/Sprint timing; crouch/slide; landing thresholds and durations; DoubleJump; slide slope; WallRun/WallLeap feel; WallCling/WallBoost timings and forces; animation fade/hold.
- **Structural (not tunable):** cast sizes and reaches (`GroundQueryCastDistance`, `GroundSupportFootprint`, `WallRunSideCast*`, `WallClingDistance`/`CastSize`, `WallDetectMaxUpDot`, `WallClingMaxNormalY`), rig facts (`RigRootHeightAboveFeet`, `TerrainVoxelSize`), `DirectionEpsilon`, `ClimbableTag`, `SlideJumpUpwardSpeed`, `WallClingDropSeparationSpeed`.
...
**Not built:**
- **Vault** (ledge detection + momentum launch). If a spatial "slide under" is ever built, don't reuse the `Slide` symbol.
